...
void HistogramDrawer::Reset(bool clearPlotLabels)
{
  ...
  if(clearPlotLabels)  plotLabels.clear();
  if(legend !=NULL  )     legend->Clear();
  if(c      !=NULL  )     c     ->Clear();
  if(pad1   !=NULL  )     pad1  ->Clear();
  if(pad2   !=NULL  )     pad2  ->Clear();
}
double getHistMin(TH1D *h, bool ignoreZero=false)
{
  ...
  for (unsigned iB=1; iB!=nB+1; ++iB)
  {
     ...
     else
    {
      ...
      if (ignoreZero && minY==0 && val>0) minY = val;
      else if (val<=0 && ignoreZero)      continue;
...
void HistogramDrawer::Draw(TString outDir, TString baseName)
{
  ...
  for (int iH=nH-1; iH!=-1; --iH)
  {
    HistWrapper w = internalHists[iH];
    TString legOption = "L";
    if (w.opt.Contains("el"))             legOption = "EL";
    if (doStack || w.opt.Contains("e2"))  legOption = "F";
    if (w.pt==kData && w.opt=="elp0")     legOption = "ELP";
  ...
  if (!doStack)
  {
    if (hOthers.size()>0)   firstHist = hOthers[0].h;
    else if (hData)         firstHist = hData;
    else
    {
      for (unsigned int iS=0; iS!=4; ++iS)
      {
  ...
  if (hData!=NULL && firstHist!=hData)    hData->Draw("elp same");
  if (legend) legend->Draw();
  c->cd();

  if (doRatio && hData) //pad2
  {
    cout << "if (doRatio nad hData)" <<endl;
    pad2->cd();
    pad2->SetGridy();
    zero = new TLine(hData->GetXaxis()->GetXmin(), 1, hData->GetXaxis()->GetXmax(), 1);
    zero->SetLineColor(1);
    zero->SetLineWidth(2);
    zero->Draw("");
    ...
    if (doDrawMCErrors && hSum)
    {
      ...
      for (int iB=1; iB!=nBins+1; ++iB)
      {
        ...
        for (auto w : internalSysts)
        {
          ...
          if (systerr>0)  mcErrUp   += pow(systerr,2);
          else            mcErrDown += pow(systerr,2);
        }
...
void HistogramDrawer::BuildRatio(const TH1D* hNum, const TH1D* hDen, TH1D*& hRatio, TGraphErrors*& gRatio, bool denUnc)
{
  ...
  hRatio->SetTitle("");
  if (padModified)  hRatio->Draw("elpsame");
  else 
  {
    hRatio->Draw("elp");
    padModified = true;
